...
show_header() {
    ...
    echo -e "${CYAN}╔════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║                                                        ║${NC}"
    echo -e "${CYAN}║${WHITE}          🔥 HAFIZ UNIQUE  🔥              ${CYAN}║${NC}"
...
EOF
    ...
    if [ -f "$FOOTER_FILE" ]; then
        sed -i 's/Hafiz Store®/Pterodactyl®/g' "$FOOTER_FILE"
...
install_wallpaper() {
    ...
    sed -i 's/Pterodactyl®/Hafiz Store®/g' "$FOOTER_FILE"
    sed -i 's/Pterodactyl/Hafiz Store/g' "$FOOTER_FILE"
    sed -i 's/© 2015 - 2026/© 2025 - 2026/g' "$FOOTER_FILE"
    sed -i 's/© [0-9]\{4\} - [0-9]\{4\}/© 2025 - 2026/g' "$FOOTER_FILE"
    
    echo -e "${GREEN}✅ Footer: Hafiz Store® © 2025 - 2026${NC}\n"
...
@section('container')
    ...
    <style>
        ...
        /* BACKGROUND */
        body {
            background: url('/hafiz/bg.jpg') no-repeat center center fixed !important;
...
EOF
    ...
    # GANTI TITLE PANEL
    # -------------------------------
    sed -i 's/<title>.*<\/title>/<title>🔥 Hafiz Store Edition 🔥<\/title>/g' "$PTERO_DIR/resources/views/layouts/admin.blade.php" 2>/dev/null
    sed -i 's/<title>.*<\/title>/<title>🔥 Hafiz Store Edition 🔥<\/title>/g' "$PTERO_DIR/resources/views/layouts/auth.blade.php" 2>/dev/null
    ...
    echo -e "${GREEN}©️  Footer       : ${NC}Hafiz Store® © 2025-2026"
    echo -e "${WHITE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
while true; do
    ...
    case $MENU in
        ...
        2)
            ...
            ;;
        x)
            echo -e "\n${GREEN}✨ Thanks for using Hafiz Store Edition!${NC}"
            echo -e "${CYAN}📱 Telegram: @Hafizzbot1 - @Hafizzbot2${NC}\n"
            exit 0
            ;;
        *)
